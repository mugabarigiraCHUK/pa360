		...
		FBModal_show2( 'proc/periode.php', 'post', "proc=add-modal", 
			true, false, null, {
				onSuccess: function(res){
					var dtpFunc = function(source, target){
						var dtp = target.retrieve('DatePicker');
						var val = parseInt(source.value)+(24*60*60);
						dtp.options.minDate = {date: val, format: 'U'};
						if ($chk(dtp.input)) dtp.input.set('value', '');
						if ($chk(dtp.visual)) dtp.visual.set('value', '');
					};
					dtpicker_attach('#periodeAwal', null, true, null, {onClose:function(){ dtpFunc($('periodeAwal'), $('periodeAkhir'));}});
					
					var pa = dtpicker_attach('#periodeAkhir', null, true, null, {onClose:function(){ dtpFunc($('periodeAkhir'), $('batasAwal'));}});
					$('periodeAkhir').store('DatePicker',pa);

					var baw = dtpicker_attach('#batasAwal', null, true, null, {onClose:function(){ dtpFunc($('batasAwal'), $('batasAkhir'));}});
					$('batasAwal').store('DatePicker',baw);

					var bak = dtpicker_attach('#batasAkhir', null, true, null);
					$('batasAkhir').store('DatePicker',bak);
